...
/****** Object:  StoredProcedure [dbo].[GET_ATTENDANCE]    Script Date: 25-02-2018 13:32:24 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[Stud_Proc]
@STARTDATE  DATETIME,
@ENDDATE    DATETIME,
@User_Id bigint
AS BEGIN
...
SET @QUERY = 'SELECT User_Id, ' + @Columns2  +' FROM
            (
                  SELECT  B.DT AS DATE, A.STATUS FROM MarkA A where User_Id=@User_Id RIGHT OUTER  JOIN #TMP_DATES B ON A.DATE=B.DT              
           ) X
            PIVOT
            (
                 MIN([STATUS])
